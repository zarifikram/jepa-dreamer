...
from utils import ContrastModel, update_state_dict, random_shift
import tools
...
class MultiEncoder(nn.Module):
    ...
    def forward(self, obs):
        if self._use_mlr_loss:
            ...
            if self.mlp_shapes:
                ...
                outputs.append(self._target_mlp(inputs))
    ...
    def calculate_atc_loss(self, obs, K:int):
        ...
        anchor = random_shift(anchor.cpu(), pad = self.pad).to(device)
        positive = random_shift(positive.cpu(), pad = self.pad).to(device)

        anchor = rearrange(anchor, "BT C H W -> BT H W C")
        positive = rearrange(positive, "BT C H W -> BT H W C")

        anchor_latent = self._cnn(anchor)
        with torch.no_grad():
    ...
    def update_target(self):
        update_state_dict(self._target_cnn, self.state_dict(), self.tau)

    def calculate_masked_non_masked_embeddings(self, masked_obs, obs):
        assert self._use_mlr_loss, "This method is only for MLR loss"
        masked_outputs, non_masked_outputs = [], []
        
        if self.cnn_shapes:
            inputs = torch.cat([masked_obs[k] for k in self.cnn_shapes], -1)
            masked_outputs.append(self._cnn(inputs))
            
            inputs = torch.cat([obs[k] for k in self.cnn_shapes], -1)
            non_masked_outputs.append(self._cnn(inputs))
            
        if self.mlp_shapes:
            inputs = torch.cat([masked_obs[k] for k in self.mlp_shapes], -1)
            masked_outputs.append(self._mlp(inputs))
            
            inputs = torch.cat([obs[k] for k in self.mlp_shapes], -1)
            non_masked_outputs.append(self._mlp(inputs))
            
        masked_outputs = torch.cat(masked_outputs, -1)
        non_masked_outputs = torch.cat(non_masked_outputs, -1)
        return masked_outputs, non_masked_outputs
    
    def on_after_backward(self):
        self.update_momentum(self.m)
        self.m += (self.m_end - self.m_start) / 1e5
    
    def update_momentum(self, m):
        if self.cnn_shapes:
            for target_param, param in zip(self._target_cnn.parameters(), self._cnn.parameters()):
...
class SPRPred(nn.Module):
    def __init__(self, input_size: int, output_size: IndentationError):
        super(SPRPred, self).__init__()
        ...
        self.global_target_classifier = copy.deepcopy(self.global_classifier)
        self.global_spr_size = output_size
        self.m_start, self.m_end = 0.5, 1.0
        self.m = self.m_start
        
    def forward(self, embed):
        embed =  self.global_classifier(embed)
        embed = self.global_final_classifier(embed)
        return embed
    
    def calculate_byol_loss(self, latents, target_latents):
        latent_shape = latents.shape
        latents = latents.reshape(-1, latents.shape[-1])
        target_latents = target_latents.reshape(-1, latents.shape[-1])
        
        self.on_after_backward()
        global_latent = self(latents)
        with torch.no_grad():
            global_targets = self.global_target_classifier(target_latents)
        
        loss = self.spr_loss(global_latent, global_targets)
    ...
    def update_momentum(self, m):
        for target_param, param in zip(self.global_target_classifier.parameters(), self.global_classifier.parameters()):
            target_param.data = m * param.data + (1 - m) * target_param.data
